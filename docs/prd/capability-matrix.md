...
| 10 | **Stage 4 "Craft"** — content in the store's own voice (bn+en), scored against a structured brand profile, revised on request | 🟡 **core built** — scoreVoice (bn-aware, cited violations) + BrandProfile + content review loop (request-changes→v2→approve) + night-shift authoring all verified; model generation (10-C), publish pipeline (10-D), Studio UI (10-F) remain | [phase-10](./capabilities/phase-10-stage4-craft.md) |
| 11–15 (v2) | Blueprint v2: one phase per Master Build PRD Stage 5–9 (Conversation → Reach → Presence → Team → Launch) | ⬜ planned | [blueprint v2](../blueprint/README.md) |
