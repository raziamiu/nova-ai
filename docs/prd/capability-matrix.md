...
| 15 | **Stage 9 "Launch hardening"** — budgets, injection defense, kill paths, grounding, GA | 🟡 **security cores built + verified (live red-team pass)** — budget shed-order (decision surfacing never sheds), untrusted() framing + red-team corpus, kill-path fail-closed/open, grounding audit (launch 20); the model does NOT act on an injected quote (live). **Remainder (H-21)**: Redis budgets, OTel/metering, LISTEN/NOTIFY, drills, ≥20-store 30-day pilot, privacy/compliance sign-off | [phase-15](./capabilities/phase-15-stage9-launch.md) |
