/**
 * L0-FOUNDER — the founder register's core prompt (Stage 10 module 02, D11).
 *
 * This is the body that used to live in `agent/instructions.md`. The root file
 * is STATIC: eve loads it for every session, so while this text sat there it
 * rendered inside customer conversations too — a customer session held two
 * contradictory role definitions ("the person you talk to is the store owner"
 * against layer 50's "the person you are talking to is a CUSTOMER"), plus the
 * trust plane's verb names, the autonomy contract and the department list.
 * Layers 10–40 were gated on `isCustomerSession`; the densest founder text of
 * all was not. Moving it here closes the module's #1 named risk.
 *
 * What stayed at the root is only what is true in BOTH registers: that Nova is
 * Nova, that no store is baked into the prompt, the never-fabricate floor, and
 * the trust boundary. Principle 6 below restates the honesty floor in founder
 * words on purpose — the numbered list is referenced by number across the
 * blueprint, so it is not renumbered around a de-duplication.
 *
 * Static text, so it resolves once per session (`session.started`) like 40;
 * a founder session sees byte-identical content to the pre-split root.
 */

import { defineDynamic, defineInstructions } from "eve/instructions";
import { isCustomerSession } from "../lib/customer/principal";

const FOUNDER_CORE = `## Who you work for

You are the AI Business Operator for this store — its full-time digital
employee, not an assistant, a copilot, or a chatbot. You run marketing, sales,
support, inventory, logistics, and finance around the clock so the founder
doesn't have to. The store's identity, goals and brand voice arrive every
session in your **Store Profile** context.
...
export default defineDynamic({
  events: {
    "session.started": (_event, ctx) =>
      isCustomerSession(ctx) ? null : defineInstructions({ markdown: FOUNDER_CORE }),
  },
});
